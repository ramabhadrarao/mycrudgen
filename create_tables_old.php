...
(1, 1), (1, 2), (1, 3), (1, 4), -- Admin Group permissions for add_page
...
// Assign permissions to admin role for all CRUD operations
$permissions = $conn->query("SELECT id FROM permissions");
while ($permission = $permissions->fetch_assoc()) {
    $permissionId = $permission['id'];
    $sql = "INSERT INTO role_permissions (role_id, permission_id) VALUES (1, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $permissionId);
    $stmt->execute();
}

echo "Admin user created with default username 'admin' and password 'nihits1981'.\n";
