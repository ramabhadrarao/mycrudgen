...
class CRUDGenerator {
    ...
    private function generateActionsPHP() {
        ...
        $content .= "                exit();\n";
        $content .= "            }\n\n";
        $content .= "            // Check for duplicate record\n";
        // Exclude foreign key fields from the duplicate check
        $columnsToCheck = array_filter($this->columns, function($col) {
            return $col !== $this->primaryKey && strpos($col, '_id') === false;
        });
        
        $content .= "            \$duplicateCheckSql = \"SELECT * FROM {$this->tableName} WHERE " . implode(' = ? OR ', $columnsToCheck) . " = ?\";\n";
        $content .= "            \$duplicateStmt = \$conn->prepare(\$duplicateCheckSql);\n";
        $content .= "            \$duplicateStmt->bind_param('" . str_repeat('s', count($columnsToCheck)) . "', " . implode(', ', array_map(fn($col) => "\${$col}", $columnsToCheck)) . ");\n";
...
$foreignKeys = [
    'student_educational_details' => [
        'student_id' => ['table' => 'students', 'key' => 'student_id', 'field' => 'name'], // Assuming you want to display student names
